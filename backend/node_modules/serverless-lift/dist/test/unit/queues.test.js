...
var CloudFormationHelpers = __toESM(require("../../src/CloudFormation"));
var import_runServerless = require("../utils/runServerless");
var import_mockAws = require("../utils/mockAws");
...
describe("queues", () => {
  ...
  it("sets the SQS visibility timeout to 6 times the function timeout + max batching window in seconds when using custom maxBatchingWindow", async () => {
    ...
    expect(cfTemplate.Resources[computeLogicalId("emails", "Queue")]).toMatchObject({
      Properties: {
        ...
        // NOTE: 5 is the maxBatchingWindow
      }
    });
    expect(cfTemplate.Resources.EmailsWorkerLambdaFunction).toMatchObject({
      Properties: {
  ...
  it("allows changing the max concurrency", async () => {
    const { cfTemplate, serverless } = await (0, import_runServerless.runServerless)({
      fixture: "queues",
      configExt: (0, import_lodash.merge)({}, import_runServerless.pluginConfigExt, {
        constructs: {
          emails: {
    ...
    const serverlessVersion = serverless.version;
