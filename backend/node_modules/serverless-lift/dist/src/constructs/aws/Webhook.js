...
class Webhook extends import_abstracts.AwsConstruct {
  constructor(scope, id, configuration, provider) {
    ...
    this.api = new import_aws_apigatewayv2.HttpApi(this, "HttpApi");
    ...
    if (resolvedConfiguration.insecure && resolvedConfiguration.authorizer !== void 0) {
      throw new import_error.default(
        `Webhook ${id} is specified as insecure, however an authorizer is configured for this webhook. Either declare this webhook as secure by removing \`insecure: true\` property (recommended), or specify the webhook as insecure and remove the authorizer property altogether.
See https://github.com/getlift/lift/blob/master/docs/webhook.md#authorizer`,
    ...
    if (!resolvedConfiguration.insecure && resolvedConfiguration.authorizer === void 0) {
      throw new import_error.default(
        `Webhook ${id} is specified as secure, however no authorizer is configured for this webhook. Please provide an authorizer property for this webhook (recommended), or specify the webhook as insecure by adding \`insecure: true\` property.
See https://github.com/getlift/lift/blob/master/docs/webhook.md#authorizer`,
        "LIFT_INVALID_CONSTRUCT_CONFIGURATION"
      );
    }
    ...
    if (!resolvedConfiguration.insecure) {
      const lambda = import_aws_lambda.Function.fromFunctionArn(
        this,
        "LambdaAuthorizer",
        import_aws_cdk_lib.Fn.getAtt(provider.naming.getLambdaLogicalId(`${id}Authorizer`), "Arn").toString()
      );
